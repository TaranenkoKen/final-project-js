import { getFilters, getExercises } from './exercises.js';
import {
  renderCategories,
  renderExercises,
  renderSkeleton,
...
const getLimit = () => {
  ...
  if (appState.view === 'categories') {
    return width < BREAKPOINTS.TABLET ? 9 : 12;
  } else {
    return width < BREAKPOINTS.TABLET ? 8 : 10;
  }
};
...
async function fetchAndRender() {
  ...
  try {
    ...
    if (appState.view === 'categories') {
      ...
      if (appState.categoryFilter === 'Muscles')
        params.muscles = appState.category.toLowerCase();
      else if (appState.categoryFilter === 'Body parts')
        params.bodypart = appState.category.toLowerCase();
      else if (appState.categoryFilter === 'Equipment')
        params.equipment = appState.category.toLowerCase();
      ...
      renderPagination(
        exercises.page ? Number(exercises.page) : 1,
    ...
    if (container)
      container.innerHTML =
        '<p class="error-message">Failed to load data. Please try again.</p>';
  }
}
