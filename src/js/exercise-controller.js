import { getExerciseById, updateRating } from './exercises.js';
import {
  openModal,
  closeModal,
  renderExerciseModal,
...
function setupExerciseModalHandlers(exerciseId, options = {}) {
  ...
  if (closeBtn) {
    closeBtn.onclick = () => closeModal('exercise-popup');
  }
...
function setupRatingModalHandlers(exerciseId) {
  const closeBtn = document.getElementById('rating-popup-close-btn');
  if (closeBtn) {
